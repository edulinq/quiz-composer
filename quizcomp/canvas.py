...
def _validate_allowed_attempts(allowed_attempts: typing.Any) -> int:
    ...
    if (not isinstance(allowed_attempts, (str, int))):
        raise quizcomp.model.errors.QuizValidationError(f"Allowed attempts must be a positive int (or -1), found '{str(allowed_attempts)}'.")

    try:
        int_allowed_attempts = int(allowed_attempts)
    except:
        raise quizcomp.model.errors.QuizValidationError(f"Allowed attempts must be a positive int (or -1), found '{str(allowed_attempts)}'.")  # pylint: disable=raise-missing-from

    if ((int_allowed_attempts < -1) or (int_allowed_attempts == 0)):
        raise quizcomp.model.errors.QuizValidationError(f"Allowed attempts must be a positive int (or -1), found '{str(int_allowed_attempts)}'.")
